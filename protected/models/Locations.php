...
class Locations extends CActiveRecord {
    ...
    public function getLocationClause($userId, $alias = 'l') {
       $result = '';
       $sql = "SELECT l.* FROM userlocations ul INNER JOIN locations l ON l.ID = ul.LocationID WHERE ul.UserId = :userid";
       $command = Yii::app()->db->createCommand($sql);
       $command->bindValue(':userid', $userId);
       ...
       foreach($rows as $row){
           if ($row['City']) {
               $result .= ' OR ' . $alias . '.City = ' . $row['City'];
           }
           elseif ($row['State']) {
               $result .= ' OR ' . $alias . '.State = ' . $row['State'];
           }
           elseif ($row['Country']) {
               $result .= ' OR ' . $alias . '.Country = ' . $row['Country'];
           }
       }
    ...
    public function GetLocationByPersonId($personId) {
        $result = null;
        $sql = 'SELECT cnt.Name country, c.Name city ' .
                'FROM persons p ' .
                'INNER JOIN household h ON p.Household = h.ID ' .
                'INNER JOIN locations l ON h.Location = l.ID ' .
                'INNER JOIN country cnt ON l.Country = cnt.ID ' .
                'INNER JOIN city c ON l.City = c.ID ' .
                'WHERE p.ID = :personId';
        $command = Yii::app()->db->createCommand($sql);
        $command->bindValue(':personId', $personId);
