...
class xhttp {
	...
	public static function request($url, $method = 'get', $requestData = array()) {
		...
		if(!isset($requestData['headers']['User-Agent']))
			$requestData['headers']['User-Agent'] = "xhttp class";
	...
	public static function utf8_encode(&$urlparts, &$requestData) {
		if(isset($requestData['get'])) 	@array_walk($requestData['get'],  array(__CLASS__, 'array_utf8_encode'));

		if(isset($requestData['post'])) @array_walk($requestData['post'], array(__CLASS__, 'array_utf8_encode'));
	}
